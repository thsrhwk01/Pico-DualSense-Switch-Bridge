...
**已在 Switch 上验证 (Verified on Switch)**

- 双击 BOOTSEL 可切换到 Pro Controller 模式。
- 基本震动正常；HD 震动的还原准确性尚未验证。
- 除麦克风按钮外，所有按钮均正常工作。

**开发中 (Under development)**

- **陀螺仪/体感存在严重错误，目前无法使用。**
- HD 震动转换仍是实验性实现，准确性和完整性仍在验证。

**尚未在 Switch 上验证 (Not yet verified on Switch)**

其余已实现的行为尚未在 Switch 上验证，包括模拟摇杆移动、麦克风按钮、
断电后模式保存、休眠/唤醒和重新连接。NFC/amiibo 和 Switch 2 的 C 按钮不受支持。

# 功能特点
 - PC 模式支持 DualSense 原生触觉反馈
 - **Switch 陀螺仪/体感：开发中，存在严重错误，目前无法使用**
