...
## Switch 진동 배율 조절 (개발 브랜치)

Switch Pro 모드에서 듀얼센스 마이크 음소거 버튼을 누른 채 십자키 ↑/↓로
진동 배율을 **1.0~2.0배, 0.1배 단위**로 조절합니다. 마이크+←는 1.0배로
초기화합니다. 한 단계마다 십자키를 떼어야 하며, 길게 누르기와 대각선은
추가 조절을 하지 않습니다. 조절 중 십자키 입력은 게임에 전달되지 않으며,
마이크 버튼을 먼저 놓아도 십자키를 뗄 때까지 차단됩니다.

마이크 LED는 변경 시 한 번, 상·하한 도달 또는 초기화 시 두 번 깜박입니다.
PC 진동 설정과 별도로 저장하며, 마이크 버튼을 놓고 마지막 변경에서 2초가
지난 뒤 저장됩니다. 저장 전에는 전원을 분리하지 마세요. 기존 설정과 페어링은
유지되고 최초 배율은 1.0배입니다. 높은 배율에서는 강한 진동 파형이 잘려
질감이 달라질 수 있습니다. 단축키와 LED 피드백의 실기 검증은 아직 필요합니다.
