...
0.8.1은 Switch 모드를 유지하면서 PC 오디오·햅틱을 upstream
`v0.7.2-hotfix` 기준으로 복원합니다. v0.8.0에서 업데이트하면 펌웨어 설정이
schema v5 기본값으로 초기화되므로 필요하면 BOOTSEL로 Switch 모드를 다시
선택하세요. Bluetooth 페어링을 지우는 변경은 포함하지 않습니다.
